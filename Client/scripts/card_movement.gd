...
func card_released() -> void:
	...
	if _over_battlefield and board.get_child_count() < Constants.MAX_BOARD:
		var side = 1
		reparent(board)
		if _play_left_side:
			...
			side = 0
		_clickable = false
		$"CardStats".on_play(side)
	else:
		var tween = create_tween()
		tween.set_trans(Tween.TRANS_SINE)
		tween.tween_property(self, "global_position", _original_position, Constants.TIME_FAILED_SUMMON_ANIM)
